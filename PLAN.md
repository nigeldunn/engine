...
**Milestone 13 Slice 4 of the app binary: COMPLETE.**

Ticket-ingest endpoint and CLI subcommand. The binary now exposes `POST /tickets` on the configured ingest address and a parallel `orchestrator-app ingest` subcommand for ops shells / automation. Both share `ingest::ingest_ticket(executor, IngestRequest)` so wire format and dedup semantics match exactly.

- `crates/orchestrator-app/src/ingest.rs`: `IngestRequest` flattens optional `workflow_id` over the canonical `TicketIngested` payload. `ingest_ticket` derives the workflow id (default `"{source}:{id}"`), runs a preflight query against the events table for the prior `workflow.ticket_ingested.v1` event under the same dedup key, and either (a) returns `IngestOutcome::AlreadyExists` for a bit-identical re-post, (b) returns `IngestError::Conflict` for a payload mismatch (operator surface for 409), or (c) calls `executor.advance(...)`. Typed errors: `Conflict`, `Preflight`, `Encode`, `Advance`. The preflight is what prevents `Storage::advance`'s silent-dedup-then-return-prior path from masking configuration drift on the same ticket id.
- `crates/orchestrator-app/src/server.rs`: new `bind_ingest_listener` + `run_ingest` + `build_ingest_router`. `POST /tickets` enforces `Authorization: Bearer <token>` when configured (constant-time compare via `subtle`); 201 on Created, 200 on AlreadyExists, 409 on Conflict (with `dedup_key` + `error: "dedup_conflict"` body), 401 on missing/wrong bearer, 400 on malformed JSON, 500 on Preflight/Advance failure. Bind happens BEFORE spawn (round-11 invariant).
- `Runtime` now carries three subsystems: `dispatcher`, `webhook_server`, `ingest_server` — all `SubsystemHandle<E>` per round-11/14 refactor. `boot` resolves the ingest bearer token + binds both listeners before any spawn. `shutdown` signals all three then drains via `tokio::join!`; `merge_outcomes` already accepts an iterator so adding the third subsystem was one extra line.
- `main.rs`: `clap` gains a `Subcommand` enum. Default behavior (no subcommand) is unchanged. `orchestrator-app ingest --source manual --id ENG-123 --repo-owner octo --repo-name world --base-branch main --base-sha <40-hex>` opens Storage directly (no Dispatcher / sinks / servers), calls `ingest_ticket`, prints the outcome, exits. Concurrent execution against a running engine is safe: SQLite WAL allows the writer contention and `Storage::advance` is transactional. Exit codes: 0 success or idempotent re-post, 2 dedup conflict, 1 other failure.
- 14 new app tests workspace-wide: 5 ingest unit (first-write, idempotent re-post, conflict, override, preflight-failure) + 7 ingest integration (201/200/409/401×2/400/override) + 2 net-new (one drop-aborts, one drained-no-warn from round-14 already counted earlier; this slice adds 12 net new given the integration side: counting carefully, the workspace went 315 → 327 = +12).
- 327 tests pass workspace-wide. `cargo build` / `cargo clippy --all-targets -- -D warnings` clean.

*Round-17 follow-up to Codex round-15/16.* `Storage::advance` now distinguishes `(workflow_id, sequence)` PK violations from `ingress_dedup_key` partial-unique-index violations: on UNIQUE error, after rollback, a fresh-pool lookup checks for an existing event under the dedup key. Match → return `AdvanceOutcome { deduplicated: true, ... }`; miss → fall through to `SequenceConflict` as before. Without this change, dedup races bounced through `Executor::advance`'s SequenceConflict retry loop and only converged on the next attempt (when the top-of-tx dedup-lookup picked up the now-visible row). Strict improvement: same external behavior on success, faster convergence on dedup races, no signature change. New `classify_unique_violation(pool, dedup_key)` helper in `crates/orchestrator-core/src/storage.rs` with 3 inline unit tests covering `Some(found)`, `None` (no dedup key), and `None` (key supplied but no row). 334 tests pass workspace-wide.

*Codex stop-gate rounds 15 + 16.* Round-15 fixed: `ingest_ticket` previously ignored `AdvanceOutcome::deduplicated`, so when a race-loser's `Storage::advance` got silently dedup'd by the unique index on `events.ingress_dedup_key`, the loser returned `Created` even though the stored payload was the winner's. Fixed by capturing the AdvanceOutcome and, when `deduplicated == true`, re-querying the stored payload via the same `check_dedup_conflict` helper used by preflight, then routing through a shared `classify_against_stored` function. Round-16 made the new branch actually testable: `ingest_ticket` is now a thin wrapper around two pub(crate) phases — `prepare_ingest` (derive ids, encode payload, run preflight) and `finalize_ingest` (advance, handle deduplicated flag, classify). Tests can call them separately and sneak a competing write between phases, deterministically driving the post-dedup branch through the production path. Tests: 2 unit tests on `classify_against_stored` (idempotent vs conflict) + 2 tests on the split phases proving Conflict and AlreadyExists outcomes via the actual `finalize_ingest` deduplicated branch. The pure concurrent in-process race against `ingest_ticket` end-to-end remains hard to engineer (snapshot isolation makes the loser hit a UNIQUE constraint → SqliteError(5) → 500, separate from this round's scope), but the split-phase test exercises every line of the new code through the real call path. 331 tests pass workspace-wide.

**v1 GitHub-driven coding workflow is now end-to-end runnable.** A single `orchestrator-app --config config.toml` boots the engine; `POST /tickets` (or the CLI) starts a workflow; the dispatcher drives it through GitHub + agent service interactions; `pull_request.closed{merged:true}` webhooks complete the cycle. The remaining work in PLAN.md ("What's next" → M11f+) is post-v1 polish and can be picked up opportunistically.

**End-to-end smoke test landed.** `crates/orchestrator-app/tests/end_to_end_smoke.rs` boots a real `Storage` + `Executor` + `Dispatcher` with the actual `WorkflowReducer`, registers two stubs (`StubAgentClient` returning canned Finished responses for the 5 agent kinds, `StubGithubSink` decoding action payloads and producing outcome events via the public `*_event` constructors in `orchestrator-github::outcome`), then ingests a ticket via `ingest_ticket` and watches the dispatcher drive itself through every stage: triage → plan → ensure_branch → code → commit → review → security → open_pr → AwaitingHumanApproval. The test then synthesizes a `PrMerged` event (skipping the webhook server layer) and confirms the workflow lands at Merged. Final assertions: (1) the cached snapshot's `WorkflowState` has `status == Merged` and `merge_commit_sha` set (Codex round-18: event presence in the log is necessary but not sufficient — `Storage::advance` always appends, even if the reducer ignores; the snapshot is the authoritative "did the reducer transition" check), (2) the exact event-type sequence so any reducer- or sink-flow drift breaks the test loudly. Runs in ~0.2s. Proves the cross-component wiring (dispatcher ↔ sinks ↔ reducer ↔ executor) that no single layer's unit tests can see in isolation.

**Milestone 13 Slice 3 of the app binary: COMPLETE.**

Webhook ingestion. The binary now spins up the github webhook HTTP server alongside the dispatcher. Inbound `pull_request.closed{merged: true}` deliveries are HMAC-validated by `orchestrator-github-webhook`, then routed via the M13 design — the app supplies a closure that resolves `WorkflowId` by querying the events table for the prior `EVT_GH_PR_OPENED` event matching `(repo, pr_number)`, then advances the workflow with a translated `PrMerged` command.

- `crates/orchestrator-app/src/webhook.rs`: `resolve_workflow_id_from_pr` runs the SQL query against `events.payload` via `json_extract`, with `COLLATE NOCASE` on owner/name (GitHub canonicalizes case server-side, so user-typed `Octo/World` must still match). `handle_delivery` pre-filters non-merge events, resolves the workflow, calls `translate_github_webhook` with the resolved id, then `executor.advance(...)`. Typed `HandleDeliveryError` distinguishes Ignored / UnresolvedWorkflow / Advance — the server module maps the first two to HTTP 200 (don't make GitHub retry forever) and the third to 500 (real failure, let GitHub retry).
- `crates/orchestrator-app/src/server.rs`: `serve_webhook(listen, prefix, secret, executor, shutdown)` mounts the github router under the configured `path_prefix` and runs `axum::serve` with `with_graceful_shutdown`. `build_webhook_router(secret, executor)` is also exposed so integration tests can drive the production handler via `tower::ServiceExt::oneshot` without binding a port.
- `Runtime` now spawns the webhook server alongside the dispatcher, each with its own `Notify` (avoiding the multi-waiter race fixed in dispatcher.rs round-6). `shutdown()` fires both notifies, awaits both joins concurrently with `tokio::join!`, and merges the per-subsystem outcomes — the worst case wins so the binary's exit code surfaces the failure mode.
- Cargo.toml: added `orchestrator-github-webhook`, `axum`, `sqlx` (sqlite), `serde_json` as direct deps; dev-deps gained `tower`, `http`, `hmac`, `sha2`, `hex` for the integration test.
- Did NOT add the partial JSON index from the M13 design discussion. v1 traffic is small enough that the existing `idx_events_payload_type` partial filter narrows the json_extract scan to the github.pr_opened.v1 subset (a few rows per ticket); the expression index is purely additive and can land later when telemetry justifies it.
- *Webhook lookup errors propagate (Codex stop-gate round-7).* `resolve_workflow_id_from_pr` returns `Result<Option<WorkflowId>, sqlx::Error>` — distinguishing "definitely no row" from "we couldn't tell". `HandleDeliveryError` gains a `Lookup { delivery_id, source }` variant; the server-side closure maps it to HTTP 500 so GitHub retries (silently mapping it to 200, as the prior code did, would drop real merge events when the DB is briefly unavailable). Tests: `returns_err_when_pool_is_closed` (resolver), `handle_delivery_surfaces_lookup_failure_when_pool_is_closed` (handler), `webhook_returns_500_when_workflow_lookup_fails` (full router via `tower::oneshot`).
- *Retry treats any error during the budget as sticky (Codex stop-gate round-10).* The earlier retry overwrote `last_err` on every successful no-row response, so a flapping DB whose budget happened to land on `Ok(None)` would silently 200 a possibly-tracked merge. Fixed by tracking `sticky_error: Option<sqlx::Error>` — once an error appears in the budget window, the deadline outcome is `Err` regardless of what comes after; only a budget with **zero** errors AND no row found returns `Ok(None)`. Refactored into a `retry_until_resolved` helper that takes a probe closure; 4 unit tests script Err / Ok(None) / Ok(Some) interleavings without needing a real DB. Regression test: `retry_surfaces_error_if_any_attempt_errored_even_when_last_was_ok_none`.
- *In-handler retry replaces "rely on GitHub retries" (Codex stop-gate rounds 8 + 9 — round 8 was wrong).* GitHub does NOT auto-retry failed webhook deliveries; 500 just marks the delivery failed in the GitHub UI without a recovery path. Real fix: `resolve_workflow_id_with_retry` polls the lookup for `LOOKUP_RETRY_BUDGET` (default 5s) with `LOOKUP_RETRY_BACKOFF` (200ms) before giving up. This holds the connection open long enough to absorb the race window between `open_pr.execute` returning success and the dispatcher writing `PrOpened` (sub-second typical, so 5s is generous). After the budget: `Ok(None)` → 200 (treat as truly untracked, GitHub stops bothering us); `Err(_)` (DB still down) → 500 (visibility-only — operator must redeliver via `POST /app/hook/deliveries/{id}/attempts` once the DB is back). `handle_delivery_with_budget(executor, delivery, budget, backoff)` exposed for tests so they don't sit through 5 production seconds. New regression test `handle_delivery_recovers_when_pr_opened_lands_during_retry_window` simulates the race by inserting `PrOpened` 30ms after the handler starts; the handler resolves mid-flight and advances.
- 308 tests pass workspace-wide (was 286 after Slice 2 + dispatcher fix; +22 = 16 webhook unit + 1 merge_outcomes + 5 integration). `cargo build` / `cargo clippy --all-targets -- -D warnings` clean.

**Codex holistic review (round-11) findings, all addressed:**

- *BLOCKER: webhook bind failures invisible.* `Runtime::boot` previously spawned the serve task before binding, so a port-in-use error would silently die in the spawned future while boot returned `Ok` and `main` waited on signals forever. Split `server.rs` into `bind_webhook_listener` (synchronous bind, surfaces `ServerError::Bind`) + `run_webhook` (consumes the bound listener). `RuntimeError::Server` wraps `ServerError`; `Runtime::boot` fails loudly on bind error. Test: `bind_returns_err_when_port_is_already_in_use`.
- *IMPORTANT: `path_prefix` validation (allow-list, Codex stop-gate rounds 11→12→13).* Round-11 only caught the missing-leading-slash case; round-12 added trailing slashes, axum-≤0.7 path params (`:id`), and wildcards (`*rest`); round-13 caught axum 0.8's brace path-parameter syntax (`{id}`, `{*x}`) my reject-list missed. Final design: `is_valid_path_prefix` allow-lists each segment to `[A-Za-z0-9._-]+` and requires `/`-separated segments — so braces, colons, asterisks, whitespace, double slashes, query/fragment markers, percent-encoding, and parens are all rejected uniformly. Test enumerates 12 reject-cases and 7 accept-cases so future axum syntax shifts won't sneak past either direction.
- *IMPORTANT: `shutdown_grace_period_ms` vs `lookup_retry_budget_ms` interaction.* If grace ≤ retry budget, a normal in-handler retry held during shutdown gets aborted as TimedOut and the operator sees a false alarm. `Config::validate` enforces strict `grace > retry_budget`. New `ConfigError::ShutdownGraceTooShort`.
- *IMPORTANT: distinct exit codes.* `main` no longer collapses DrainErrored and Panicked. Mapping is now: `0` clean, `1` drain errored, `2` shutdown timed out (k8s SIGKILL window), `3` task panicked. Documented in main.rs.
- *IMPORTANT (slice-4 prep): ingest listen defaults to loopback.* `IngestServerConfig.listen` now has `#[serde(default)] = "127.0.0.1:8081"`, matching the M13 design's loopback-by-default + non-loopback-needs-bearer-token rule (already enforced in slice 1's `validate`).
- *NIT: configurable retry budget + backoff.* `LOOKUP_RETRY_BUDGET` / `LOOKUP_RETRY_BACKOFF` constants removed. New `[server.webhook].lookup_retry_budget_ms` (default 5000) and `lookup_retry_backoff_ms` (default 200) plumbed through `Runtime::boot` → `run_webhook` → `build_webhook_router` → `handle_delivery_with_budget`. Production wrapper `handle_delivery` (which used the constants) is gone — production callsites now always pass the configured values.
- *NIT: loaded-config wrapper.* `Config::load` now returns `LoadedConfig`, a newtype that pairs the schema with the validated `base_dir`. `Runtime::boot(&LoadedConfig)` reads both from one place — eliminating the drift risk where main passed `(cfg, base_dir)` separately and a future caller could mismatch them. `LoadedConfig: Deref<Target = Config>` so existing `cfg.dispatcher` access continues to work.
- *Partial-boot leak fix (Codex stop-gate round-14).* `Runtime::boot` previously spawned the dispatcher *before* resolving the webhook secret and binding the listener; if either failed we returned `Err` but the dispatcher JoinHandle was dropped without abort, leaving a stranded task processing actions with no observer. Two-pronged fix: (1) reordered boot so every fallible step (`resolve` calls + `bind_webhook_listener`) happens *before* any `tokio::spawn` — from the first spawn onward, boot is infallible; (2) added a `Drop` guard on `SubsystemHandle` that aborts the join if it was never drained. The handle's `join` is now `Option<JoinHandle<...>>` so `drain` can `take()` it (Drop becomes a no-op on the normal path) — required because `impl Drop` blocks partial moves out of the original field. Tests: `dropping_handle_without_drain_aborts_task` (verifies the guard fires using `AbortHandle::is_finished`) and `drained_handle_does_not_warn_on_drop` (verifies the normal-path Drop is a no-op).
- *Subsystem-handle refactor (deferred from round-11, landed before slice 4).* `Runtime` no longer carries 4 separate fields per subsystem (shutdown notify + join handle, two of each). New generic `SubsystemHandle<E>` bundles `(name, shutdown notify, JoinHandle<Result<(), E>>)` and exposes `signal_shutdown()` + `drain(grace)`. Eliminated the duplicated `await_dispatcher_drain` / `await_server_drain` helpers. Generalized `merge_outcomes` to take an `IntoIterator<Item = SubsystemDrain>` so adding the slice-4 ingest server is one field + one spawn + one extra arm in `tokio::join!`. Severity ranking now matches main.rs's exit codes: `Drained(0) < DrainErrored(1) < TimedOut(2) < Panicked(3)` — Panicked is more severe than TimedOut because it's a code defect rather than a possibly-transient slow handler.
- 312 tests pass workspace-wide (was 308; +4 net = 1 bind-failure + 2 path_prefix + 1 ingest default; the LoadedConfig refactor + retry-config plumbing retained existing test coverage). `cargo build` / `cargo clippy --all-targets -- -D warnings` clean.

Next: Slice 4 — ticket-ingest endpoint + CLI subcommand. `POST /tickets` accepts a `TicketIngested` JSON body with optional `workflow_id` override; default `workflow_id = "{source}:{id}"` and matching `ingress_dedup_key`. Repeat post with conflicting payload → 409. Auth: bind 127.0.0.1 by default; non-loopback listen requires the `[server.ingest].bearer_token` (already enforced at config-validate time in slice 1). Add the same subcommand surface to `clap` so operators can `orchestrator-app ingest --ticket-source manual --ticket-id ENG-123 ...` against a running binary's HTTP endpoint or directly via the executor.

**Milestone 13 Slice 2 of the app binary: COMPLETE.**

Engine boot wiring. The binary now opens `Storage` from the configured (config-dir-relative) sqlite path, builds `Executor::new(storage, WorkflowReducer)`, builds `Dispatcher` with config-driven `poll_interval` / `health_check_interval` / `sink_unhealthy_retry_delay` (other dispatcher knobs stay at core defaults until we have telemetry to tune them), registers `GithubSink` + `GithubHintExtractor` + `AgentRunnerSink<HttpAgentClient>`, spawns the dispatcher loop, and waits for Ctrl+C / SIGTERM before triggering graceful shutdown via the dispatcher's `Notify`.

- New module `src/runtime.rs` exposes `Runtime::boot(&Config, base_dir) -> Result<Runtime, RuntimeError>`. RuntimeError wraps `ConfigError`, `ExecutorError`, and `GithubAuthError` via `#[from]`. `Runtime::shutdown(self)` fires the notify, awaits the dispatcher join, and swallows panics so the binary exits cleanly on a misbehaving sink.
- Workspace cross-crate deps added to `crates/orchestrator-app/Cargo.toml`: `orchestrator-core`, `orchestrator-github`, `orchestrator-coding-workflow`, `orchestrator-agent-runner`.
- `main.rs` builds a multi-thread tokio runtime explicitly (no `#[tokio::main]`) so the `WorkerGuard` from `logging::init` outlives the runtime block. Signal wait uses `tokio::signal::ctrl_c` plus `unix::SignalKind::terminate` on Unix; `cfg(not(unix))` falls back to ctrl_c only.
- Config schema: dropped dead `github.sink_key` field — `GithubSink` hardcodes `"github"` in v1 (multi-installation support is the deferred design question further down). Storage path resolution mirrors the secret-path fix from Slice 1: relative `sqlite_path` resolves against the config file's directory, not the process cwd. New `StorageConfig::resolved_sqlite_path(&self, base_dir) -> PathBuf` helper; covered by 2 tests.
- `Runtime::shutdown` returns a `ShutdownOutcome` enum (`Drained` / `DrainErrored` / `Panicked` / `TimedOut`) and bounds the wait by `cfg.dispatcher.shutdown_grace_period_ms` (default 25 000ms — under k8s' 30s SIGKILL window so we exit before the kernel kills us). On grace-period expiry the dispatcher join is aborted and `main` exits 2 (Codex stop-gate round-5: an unbounded join would let a stuck sink handler hold the binary open forever). 2 tests cover the helper directly: `drain_completes_within_grace_period` and `drain_exceeding_grace_period_aborts_and_reports_timeout`.
- *Dispatcher shutdown bug fixed (Codex stop-gate round-6).* `Dispatcher::shutdown_handle()` is a single `Arc<Notify>` shared between the main run loop and the health-check loop. A caller's `notify_one()` only wakes ONE waiter — whichever lost the race kept blocking forever, and the main loop's `health_handle.await` hung indefinitely (which the runtime grace-period timeout would then trip on every shutdown). Fix in `crates/orchestrator-core/src/dispatcher.rs`: each loop's shutdown branch now calls `notify_waiters()` (wake any peer currently pending) followed by `notify_one()` (leave a permit for a peer that hasn't re-registered yet). New regression test `dispatcher_shutdown_completes_promptly` spawns the dispatcher, fires `notify_one()`, and asserts the join resolves within 2s — it now drains in ~100ms.
- 286 tests pass workspace-wide (was 281 after Slice 1; +5 = 2 sqlite_path + 2 drain + 1 dispatcher shutdown). `cargo build` / `cargo clippy --all-targets -- -D warnings` clean.

Next: Slice 3 — webhook ingestion. Mount `orchestrator-github-webhook::router(...)` on `cfg.server.webhook.listen`; the handler closure resolves `WorkflowId` by querying the events table for `EVT_GH_PR_OPENED` payloads matching `(repo, pr_number)` from the delivery (per the M13 design — no PR-body markers), then calls `executor.advance` with the translated `EventCommand`. Add a partial JSON index on `events.payload` to keep the lookup cheap.

**Milestone 13 Slice 1 of the app binary: COMPLETE.**

Crate skeleton + config + logging. New crate `orchestrator-app` (under `crates/orchestrator-app/`) boots a `--config <PATH>`-driven binary that loads the M13-designed TOML config, validates it, initializes tracing-subscriber, and logs a boot stub. Engine wiring is Slice 2.

- `Cargo.toml` — deps for slice 1 only: `tokio`, `clap` (derive), `figment` (toml + env), `serde`, `tracing`, `tracing-subscriber` (env-filter + json), `tracing-appender`, `thiserror`. Workspace cross-crate deps come in slice 2.
- `src/config.rs` — full schema (`storage`, `github`, `agent_runner`, `server.{webhook,ingest}`, `dispatcher`). `Secret` is an untagged enum (`{ inline = "..." }` or `{ path = "..." }`); deserialize-time validation rejects both-set and neither-set. `#[serde(deny_unknown_fields)]` everywhere. `Config::load(path)` layers TOML → env (`ORCH_<SECTION>__<KEY>`). Post-load `validate()` enforces the security boundary: ingest server on a non-loopback address requires a bearer token (M13 design).
- `src/logging.rs` — `tracing-subscriber` with `EnvFilter` (default `info,orchestrator=debug`), pretty when stdout is a TTY, JSON otherwise. `tracing-appender::non_blocking` keeps log writes off the runtime. Returns a `WorkerGuard` that `main` holds for process lifetime.
- `src/main.rs` — `clap` parses `--config <PATH>` (no implicit search path per M13 design), then load + log + exit.
- `src/lib.rs` — thin re-export of `config` and `logging` so the schema is testable.
- 14 config tests cover happy load, default `path_prefix`, both-secret-fields rejected, neither-rejected, unknown-top-level-field rejected, non-loopback-ingest-without-token rejected, non-loopback-ingest-with-token accepted, plus full empty-secret coverage (Codex stop-gate round-3): inline empty rejected, path-pointing-to-empty-file rejected, path-pointing-to-missing-file rejected with IO error, optional `agent_runner.bearer_token` empty-rejected, optional `server.ingest.bearer_token` empty-rejected even on loopback. `Secret::resolve` materializes content (file read for `path` variant) at validate time and errors on empty contents — `is_some()` is not enough, an empty inline string silently disables auth. **Relative `path` values resolve against the config file's directory, not the process cwd** (Codex stop-gate round-4: cwd is brittle under systemd/docker). Absolute paths unchanged.
- 281 tests pass workspace-wide (was 267 pre-slice + 14 new). `cargo build` / `cargo clippy --all-targets -- -D warnings` clean.

Next: Slice 2 boots the engine — open `Storage`, build `Executor::new(storage, WorkflowReducer)` and `Dispatcher`, register `GithubSink` + `AgentRunnerSink<HttpAgentClient>`, spawn the dispatcher loop, install a signal handler that fires the dispatcher's shutdown `Notify`.
...
**Resolved design decisions for M13** (Codex-reviewed 2026-05-09):

- *Workflow-id from webhooks — query the `EVT_GH_PR_OPENED` event log.* The webhook crate translator (`orchestrator-coding-workflow/src/webhook.rs::translate_github_webhook`) keeps its existing transport-only `resolve_workflow_id` closure surface unchanged; the app binary supplies a closure that runs a SQL query against the events table for `payload_type = 'github.pr_opened.v1'` matching `(repo.owner, repo.name, pr_number)` extracted from the GitHub webhook payload. The data is already there: the github sink's `PrOpened` outcome event payload carries `repo` + `pr_number`, and the wrapping `EventCommand` carries `workflow_id`. For v1 the query goes against the events table directly with a partial index on `(payload_type, json_extract(payload, '$.repo.owner'), json_extract(payload, '$.repo.name'), json_extract(payload, '$.pr_number'))`; if perf becomes an issue we can materialize a `pr_routes` read model later by tailing PrOpened events (purely additive). **No PR-body markers** (Codex round-1: PR bodies are mutable, no uniqueness proof comparable to action_id's blake3 derivation, and the webhook translator was correctly designed transport-only — keep marker/index logic out of the library).

  *Code prerequisite, applied 2026-05-09:* `apply_pr_merged` (reducer.rs) now checks `(repo, pr_number)` against state — a webhook for a fork or unrelated repo with a colliding PR number can no longer falsely complete the workflow. Repo comparison uses `RepoRef::eq_ignore_ascii_case` (Codex stop-gate round-2: GitHub normalizes owner/name in API responses, so user-typed `Octo/World` from a ticket payload must still match the canonical `octo/world` carried by the webhook). Tests: `pr_merged_for_wrong_repo_is_ignored` in `tests/happy_path.rs`; `repo_ref_eq_ignore_ascii_case_matches_canonical_variants` in `crates/orchestrator-github/src/action.rs`.

- *Ticket-ingest API — HTTP `POST /tickets` + CLI `ingest` subcommand sharing one translator.* Body is a `TicketIngested` JSON document plus an **optional** `workflow_id` override field. When omitted, `WorkflowId` defaults to `format!("{source}:{id}", ticket.source, ticket.id)` and `ingress_dedup_key` to the same — re-POSTs are idempotent for the typical one-ticket-one-workflow case. Callers who want intentional fan-out (a fresh retry workflow for a ticket that previously failed) supply an explicit `workflow_id` and matching dedup key. **Re-POST with conflicting payload returns `409 Conflict`** (Codex round-1: silent dedup-then-ignore would mask config drift — same ticket key with a different repo/base is a callsite bug, surface it). Auth posture: ingest endpoint binds to `127.0.0.1` by default; that **is** the security boundary in v1. Operators who want the endpoint network-reachable must set `[server.ingest].bearer_token{,_path}` AND a non-loopback `listen` address — startup fails if the listen address is non-loopback without a token.

- *Configuration — TOML file via `serde` + `figment`* with file → env → CLI overlay (figment standard env pattern: `ORCH_<SECTION>__<KEY>`). **Required `--config /path/to/config.toml` flag — no implicit search path** (Codex round-1: cwd-vs-`/etc` precedence is a footgun; explicit beats convention). Secrets (GitHub PEM, webhook secret, agent bearer token) modeled as a serde-untagged enum with exactly two variants — `{ inline: String }` or `{ path: PathBuf }` — and validated at deserialize time (exactly one variant present, unknown fields rejected via `#[serde(deny_unknown_fields)]` on the enclosing config struct). Schema sections: `[storage]`, `[github]`, `[agent_runner]`, `[server.webhook]`, `[server.ingest]`, `[dispatcher]`.

- *Logging — `tracing-subscriber` initialized once in the app binary's `main`.* Library crates do **not** depend on `tracing-subscriber` as a normal dep; it stays a dev-dependency for tests only (Codex round-1; `orchestrator-core/Cargo.toml` audit landed 2026-05-09 — the other library crates already had it correctly placed under `[dev-dependencies]`). `EnvFilter` reads `RUST_LOG` (default `info,orchestrator=debug`); pretty formatter when stdout is a TTY, JSON formatter otherwise (production deploys pipe stdout to a log shipper). Span enter/exit at `DEBUG` so existing `#[instrument]` annotations produce correlated structured lines without spamming the dev terminal. No OTLP / opentelemetry in v1 — that's the post-v1 observability item.

**Codex review items deferred** (don't block M13 implementation):

- *Foreclosure on multi-workflow-per-ticket fan-out.* The optional `workflow_id` override field on the ingest API is the v1 escape hatch. A first-class `/tickets/{id}/restart` endpoint that auto-derives a fresh workflow id from the existing one (e.g., `{source}:{id}#run-2`) can land later when a real use case arrives.
- *Marker-payload escaping.* If we ever revisit markers (we shouldn't), validate ticket source/id against `[A-Za-z0-9_:-]+` at ingest time so they can't break HTML-comment syntax. Not urgent without markers.
- *Admin recovery path for unrouted webhooks.* The events-query approach largely eliminates this (no mutable surface to break), but if a webhook fails to route — e.g., the `PrOpened` event was never written because the open_pr action was still in flight when the merge fired — operators may want a CLI to inject a `PrMerged` event by `(workflow_id, repo, pr_number)`. Build when first needed.
